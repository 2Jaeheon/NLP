...
class BPETokenizer:
  ...
  def evaluate_word_coverage(self, input_path):
    total_words = 0  # 전체 단어
    single_tokenized = 0  # 하나의 토큰으로 구성된 단어
    ...
    with open(input_path, 'r', encoding='utf-8') as infile:
      for line in infile:
        ...
        for word in words:
          ...
          tokens = self.apply_BPE(word)
          total_words += 1
          # 하나의 토큰으로 구성되는 경우 카운트 증가
          if len(tokens) == 1:
            single_tokenized += 1

    # 전체 단어 중 하나의 토큰으로만 구성된 단어 비율 계산
    coverage = single_tokenized / total_words * 100
...
if __name__ == '__main__':
  ...
  # 추론 모드
    ...
    tokenizer.evaluate_word_coverage(args.input)
